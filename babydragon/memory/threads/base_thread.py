...
class BaseThread:
    ...
    def load(self, path: Union[str, None]) -> None:
        if path is None:
            path = os.path.join(self.save_path, f"{self.name}.parquet")
        self.memory_thread = pl.read_parquet(
            source=path,
            use_pyarrow=True,
            memory_map=True,
        )
    ...
    def dict_to_row(self, message_dict: Dict[str, Any]) -> pl.DataFrame:
        return pl.DataFrame(
            ...
            data={
                ...
                "tokens_count": [
                    len(self.tokenizer.encode(message_dict["content"])) + 7
                ],
            },
        )
    ...
    def add_dict_to_thread(
        self, message_dict: dict, conversation_id: str = None
    ) -> None:
        """
        ...
        if (
            ...
            self.memory_thread = pl.concat(
                [self.memory_thread, new_message_row], rechunk=True
            )
            self.total_tokens = self.get_total_tokens_from_thread()
        else:
    ...
    def remove_dict_from_thread(
        ...
        if message_dict is None and idx is None:
            ...
            self.memory_thread = (
                self.memory_thread.lazy()
                .with_row_count("index")
                .filter(pl.col("index") != idx)
                .drop("index")
                .collect()
            )
            self.total_tokens = self.get_total_tokens_from_thread()
            ...
            self.memory_thread = (
                self.memory_thread.lazy()
                .filter(self.memory_thread["content"] != message_dict["content"])
                .collect()
            )
            self.total_tokens = self.get_total_tokens_from_thread()
            return
    ...
    def find_message(self, message: Union[dict, str]) -> pl.DataFrame:
        """
        ...
        if isinstance(message, str):
            return (
                self.memory_thread.lazy().filter(pl.col("content") == message).collect()
            )
            ...
            return (
                self.memory_thread.lazy()
                .filter(
                    (pl.col("content") == message["content"])
                    & (pl.col("role") == message["role"])
                )
                .collect()
            )
    ...
    def last_message(self, role: Union[str, None] = None) -> Optional[pl.DataFrame]:
        ...
        if role is None:
            ...
            return filtered_messages.tail(
                1
            )  # Return the last row of the filtered DataFrame as a DataFrame
    ...
    def messages_between(self, start_message: dict, end_message: dict) -> pl.DataFrame:
        """
        ...
        start_index = (
            ...
            .with_row_count("index")
            .filter(
                (pl.col("content") == start_message["content"])
                & (pl.col("role") == start_message["role"])
            )
            .select("index")
            .collect()[0][0]
        )
        end_index = (
            ...
            .with_row_count("index")
            .filter(
                (pl.col("content") == end_message["content"])
                & (pl.col("role") == end_message["role"])
            )
            .select("index")
            .collect()[0][0]
        )
        return (
            self.memory_thread.lazy()
            .with_row_count("index")
            .filter((pl.col("index") < end_index) & (pl.col("index")) > start_index)
            .collect()
        )

    def messages_more_tokens(
        self, tokens: int, role: Union[str, None] = None
    ) -> pl.DataFrame:
        """
        ...
        if role is None:
            return (
                self.memory_thread.lazy()
                .filter(pl.col("tokens_count") > tokens)
                .collect()
            )
        else:
            return (
                self.memory_thread.lazy()
                .filter((pl.col("role") == role) & (pl.col("tokens_count") > tokens))
                .collect()
            )

    def messages_less_tokens(
        self, tokens: int, role: Union[str, None] = None
    ) -> pl.DataFrame:
        """
    ...
    def messages_after_time(
        self, timestamp: int, role: Union[str, None] = None
    ) -> pl.DataFrame:
        """
        Get all messages with more tokens than a specific number in the memory thread with a specific role.
        """
        ...
        return (
            self.memory_thread.lazy()
            .filter((pl.col("role") == role) & (pl.col("timestamp") > timestamp))
            .collect()
        )

    def messages_before_time(
        self, timestamp: int, role: Union[str, None] = None
    ) -> pl.DataFrame:
        """
        Get all messages with more tokens than a specific number in the memory thread with a specific role.
    ...
    def token_bound_history(self, max_tokens: int, role: Union[str, None] = None):
        reversed_df = self.memory_thread.lazy().with_row_count("index").reverse()
        reversed_df = reversed_df.with_columns(
            pl.col("tokens_count").cumsum().alias("cum_tokens_count")
        )
        filtered_df = reversed_df.filter(
            (pl.col("cum_tokens_count") <= max_tokens) & (pl.col("role") != role)
        ).collect()
    ...
    def load_from_gpt_url(self, url: str):
        ...
        if response.status_code == 200:
            ...
            raise ValueError(
                f"Non è stato possibile accedere alla pagina. Codice di stato: {response.status_code}"
            )
        ...
        if next_data is not None:
                ...
                try:
                    if c["mapping"][m]["message"] is not None:
                        new_row = {
                            ...
                            "content": c["mapping"][m]["message"]["content"]["parts"][
                                0
                            ],
                        }
            ...
            raise ValueError("Nessuna conversazione trovata a questo link!")
