...
class SubjectPerspectiveAnalyzer:
    ...
    def analyze_subject_perspective(
        self, user_subject: str, user_perspective: str
    ) -> dict:
        prompts = [
        ...
        with RateLimitedThreadPoolExecutor(
            max_workers=3, calls_per_minute=20, verbose=False
        ) as executor:
            future_to_prompt = {
                executor.submit(self._analyze_prompt, prompt): prompt
            ...
            for future in as_completed(future_to_prompt):
                ...
                try:
                    ...
                    counter = 0
                    print(
                        f'An exception occurred while analyzing prompt "{prompt}": {exc}'
                    )
                    while counter < 3:
                        try:
...
class IdeaCluster:
    ...
    def create_idea_index(self):
        ...
        idea_index = MemoryIndex(
            values=self.gathered_docs, is_batched=True, name="ideas"
        )
        return idea_index
    ...
    def create_minimum_spanning_paths(self):
        if self.cluster_labels is None:
            raise ValueError(
                "You must run cluster_embeddings() before creating minimum spanning paths."
            )
    ...
    def get_clustered_ideas(self):
        if self.cluster_labels is None:
            raise ValueError(
                "You must run cluster_embeddings() before getting clustered ideas."
            )
...
class Summarizer:
    ...
    def summarize_texts(self) -> dict:
        ...
        with RateLimitedThreadPoolExecutor(
            max_workers=12, calls_per_minute=200, verbose=False
        ) as executor:
            future_to_text = {
                executor.submit(self._summarize_text, text): text for text in self.texts
            }
...
def generate_perspective_prompt(
    user_subject, user_perspective, seed_model="gpt-3.5-turbo"
):
    start = perf_counter()
    ...
    start = perf_counter()
    index = MemoryIndex(
        name="wiki_index", load=True, is_batched=True, embedder=CohereEmbedder
    )
    ...
    if not loaded:
        ...
        index = MemoryIndex.from_hf_dataset(
            dataset_url,
            ["title", "text"],
    ...
    print(
        "Number of tokens in each cluster: ", [len(tokens) for tokens in ideas_tokens]
    )
    start = perf_counter()
...
class PerspectivePromptGenerator:
    def __init__(
        ...
        self.executor = RateLimitedThreadPoolExecutor(
            max_workers=max_workers, calls_per_minute=calls_per_minute
        )
        self.prompts = []
        ...
        for subject in self.subjects:
            for perspective in self.perspectives:
                future = self.executor.submit(
                    generate_perspective_prompt, subject, perspective
                )
