...
from babydragon.models.generators.polars_batch_generator_models import (
    OpenaiRequestModel,
    StatusTrackerModel,
)
...
class PolarsGenerator:
    def __init__(
        ...
        if isinstance(input_df, pl.DataFrame):
            ...
            raise TypeError(
                "Constructor requires either a pl.Dataframe or a path to a ndjson"
            )
        ...
        while True:
            ...
            if next_request is None:
                try:
                    ...
                    self.st.replace(
                        "num_tasks_started", self.st["num_tasks_started"] + 1
                    )
            ...
            if next_request is None:
                try:
                    ...
                    logging.info(
                        f"Next request is {next_request[0]} of {self.len_queue}"
                    )
            ...
            if next_request is not None:
                ...
                if (
                    ...
                    try:
                        ...
                        async with aiohttp.ClientSession() as session:
                            async with session.post(
                                url=next_request[1].url,
                                headers=self.request_header,
                                json=next_request[1].body,
                            ) as response:
                        ...
                        if "error" in response:
                            ...
                            if "Rate limit" in response["error"].get("message", ""):
                                ...
                                self.st.replace(
                                    ...
                                    self.st["num_rate_limit_errors"] + 1,
                                )
                            ...
                            elif "currently overloaded" in response["error"].get(
                                "message", ""
                            ):
                                self.available_request_capacity += 1
                                self.available_token_capacity = (
                                ...
                                self.st.replace(
                                    "num_overloaded_errors",
                                    self.st["num_overloaded_errors"] + 1,
                                )
                                self.retries_queue.put_nowait(next_request)
                                ...
                                self.st.replace(
                                    "num_api_errors", self.st["num_api_errors"] + 1
                                )
                            ...
                            self.available_token_capacity = int(
                                headers["x-ratelimit-remaining-tokens"]
                            )
                            self.available_request_capacity = remaining_token_capacity
                            reset_time_token_capacity = headers[
                                "x-ratelimit-reset-tokens"
                            ]
                            logging.info(
                                f"From Headers: Available_token_capacity changed to {self.available_token_capacity} for request with id {next_request[0]}"
                            )
                            if "ms" in reset_time_token_capacity:
                                self.reset_time_token_capacity = (
                                    float(reset_time_token_capacity.replace("ms", ""))
                                    / 1000.0
                                )
                            elif "s" in reset_time_token_capacity:
                                self.reset_time_token_capacity = float(
                                    reset_time_token_capacity.replace("s", "")
                                )
                            else:
                            ...
                            if self.available_token_capacity < 3000:
                                await asyncio.sleep(
                                    self.reset_time_token_capacity
                                    / self.max_power_process
                                )
                            else:
                                max_power_process = self.max_power_process + 1
                                if (
                                    max_power_process
                                    >= self.process_objects_number * 10000
                                ):
                                    self.max_power_process = (
                                        self.process_objects_number * 10000
                                    )
                            ...
                            logging.info(
                                f"Max power process is :{self.max_power_process}"
                            )

                    except Exception as e:
                        logging.warning(
                            f"Request {next_request[0]} failed with Exception {e}"
                        )
                        self.st.replace(
                            "num_other_errors", self.st["num_other_errors"] + 1
                        )
                        self.available_token_capacity = (
                            self.available_token_capacity + next_request_tokens
                        )
                        ...
                        output = {"type": "other", "time": now(), "response": str(e)}
                    ...
                    seconds_since_rate_limit_error = (
                        now() - self.time_of_last_rate_limit_error
                    )
                    if seconds_since_rate_limit_error < 3:
                        self.max_power_process = self.process_objects_number
                        logging.info(
                            f"Max power process decreased to :{self.max_power_process}"
                        )
                        time_to_wait = (
                            self.reset_time_token_capacity
                            - seconds_since_rate_limit_error
                        ...
                        logging.warn(
                            f"Pausing to reset_time_token_capacity = {time_to_wait/self.max_power_process}"
                        )
                        ...
                        await asyncio.sleep(
                            self.reset_time_token_capacity / self.max_power_process
                        )
                        self.available_token_capacity = (
                            self.available_token_capacity + next_request_tokens
                        )
    ...
    async def main(self):
        ...
        self.consumers = [
            asyncio.create_task(self.process_objects())
            for _ in range(self.process_objects_number)
        ]
        ...
        for consumer in self.consumers:
            consumer.cancel()
        logging.info(
            f"""Parallel processing complete. Results saved to {self.save_path}"""
        )
        if self.st["num_rate_limit_errors"][0] > 0:
            logging.warning(
                f"{self.st['num_rate_limit_errors'][0]} rate limit errors received. Consider running at a lower rate."
            )
