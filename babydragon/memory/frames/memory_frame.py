...
from babydragon.memory.frames.frame_models import MemoryFramePydantic
from babydragon.models.generators.PolarsGenerator import PolarsGenerator
from babydragon.utils.dataframes import (
...
class MemoryFrame(BaseFrame):
    def __init__(
        ...
        self.frame_template = MemoryFramePydantic(
    ...
    def get_overwritten_attr(self):
        ...
        memory_frame_methods = [
            method
            for method in dir(MemoryFrame)
            if callable(getattr(MemoryFrame, method))
        ]
    ...
    def embed_column(
        self, column: str = "content", generator_log_name: str = "memory_embedding"
    ):
    ...
    def generate_column(
        self,
        column_name: str,
        generator_log_name: str = "memory_summary",
        model_name: str = "gpt-3.5-turbo-16k",
        system_prompt: str = "Youre a Helpful Summarizer!",
    ):
    ...
    def search_column_with_sql_polar(
        self, sql_query, query, embeddable_column_name, top_k: int
    ):
    ...
    def search_column_polar(self, query, embeddable_column_name, top_k: int):
        embedding_column_name = "embedding|" + embeddable_column_name
